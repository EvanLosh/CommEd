...
def seed_database():
    with app.app_context():
        ...
        for i in range(8):
            ...
            db.session.add(user)
        # db.session.add(User(username='dev', email='dev@fake-email.com'))
        db.session.commit()

        tag_proofs = Tag(text = 'Proofs')
        tag_number_theory = Tag(text = 'Number theory')
        tag_integers = Tag(text = 'Integers')
        tag_vectors = Tag(text = 'Vectors')
        tag_optics = Tag(text = 'Optics')
        tag_focal_length = Tag(text = 'Focal length')
        tag_lenses = Tag(text = 'Lenses')
        tag_current = Tag(text = 'Current')
        tag_circuits = Tag(text = 'Circuits')
        tag_v_equals_ir = Tag(text = 'V = IR')
        tag_Newtons_law = Tag(text = "Newton's Laws")
        tag_sigma_f = Tag(text = 'ΣF')
        tag_probablity = Tag(text = 'Probability')
        tag_series = Tag(text = 'Series')
        tag_Fourrier = Tag(text = 'Fourrier Transform')
        tag_euler_identity = Tag(text = 'Euler identity')
        tag_complex_numbers = Tag(text = 'Complex numbers')
        tag_chemistry = Tag(text = 'Chemistry')
        tag_decay = Tag(text = 'Decay')
